...
Servo Heater;
PID Controller(&temp, &dbout, &goalTemp, 2, 1, 1, DIRECT);
...
void loop() {
  ...
  temp = (0.000000000000005 * pow(sensorValue, 6)) - (0.00000000001 * pow(sensorValue, 5)) + (0.00000001 * pow(sensorValue, 4)) - (0.000005 * pow(sensorValue, 3)) + (0.0011 * pow(sensorValue, 2)) + (0.0359 * sensorValue) - 2.3354;
  Controller.Compute();
      ...
      case 0:
        switch (startChar) {
          ...
          case 's':
            ...
            if (result > 0) {
              goalTemp = constrain(newTemp.toInt(), 0, 140);
              Output = "Temp set to : " + String(goalTemp);
            } else {
              Output = "Please enter a valid Temp <0 - 140>";
            }
            OutputTimer = 10000;
